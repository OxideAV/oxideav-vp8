...
//!
//! * **VP8 encoder Phase 1**: the §7.3 [`encoder::BoolEncoder`] (a Rust
//!   port of the reference `bool_encoder` C listing embedded in RFC
//!   6386), the §9.1 / §9.3 / §9.4 / §9.5 / §9.6 / §9.9 / §9.10 / §9.11
...
//!   §13 token / §14 transform / mode-selection logic isn't exercised
//!   yet — but the resulting bytes are a structurally-valid VP8 key
//!   frame the crate's own decoder and an external `ffmpeg -c:v vp8`
//!   both accept. See [`encoder`].
