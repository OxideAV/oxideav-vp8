...
| `simd` | — | **Nightly-only.** Switches the §14.3 / §14.4 4×4 transform primitives — both inverse partners (`inverse_wht_4x4`, `inverse_dct_4x4`) and, as of round 226, both forward partners (`forward_wht_4x4`, `forward_dct_4x4`) — over to `core::simd::Simd<i32, 4>` rewrites. Every SIMD primitive is byte-exact against the scalar path on a 21-input stress set (DC-only across 10 magnitudes, single-AC at every position, mixed gradients). Headline micro-bench numbers on `aarch64-apple-darwin` (criterion `--quick`): inverse WHT 9.4 → 7.5 ns (≈ −20 %); inverse DCT 10.07 → ~9.5 ns (≈ −1 to −5 %); forward WHT 10.74 → 8.72 ns (≈ −19 %); forward DCT 10.71 → 11.54 ns (≈ +8 %, the multiply-heavy DCT plus the lane-wide symmetric `round_div2` chain costs more per call than the scalar path's straightline arithmetic does — kept on the SIMD path for shape parity with the WHT; a future round can split the dispatch). Requires a nightly toolchain because `core::simd` is itself nightly. Default (stable) builds use the scalar §14.3 / §14.4 paths unchanged. See `BENCHMARKS.md` for the A/B numbers + the round-170 / round-180 / round-226 profiles that motivated each primitive's vectorisation. |
